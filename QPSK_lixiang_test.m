function QPSK_lixiang_test
clc;
clear all;
global N_48;N_48 = 48;                          %用户路数
global frame_num;frame_num = 4;                      %每一路子帧数
global data_L;data_L = frame_num*278;             %每一路数据长度
...
[Ii,Qi] = create_signal(data_L,N_48);                           %产生QPSK调制信号
Io = lvbo_shape_shen(Ii+1i*Qi);                                      %成形滤波，两倍上采样
...
for kk = 1:length(snr)
    for ll = 1:300
        SNR=10^((snr(kk)+10*log10(2)+10*log10(1/1.25)+0.4)/10);%信噪比
        ...
        data_r = match([noise(1:5000) Send_data]);               %匹配滤波
        ...
        data_yuan(1:data2_L,:) = sign(real(data_fc));error1 = sum(data_yuan(1:data2_L,:)~=Ii);  %%解调
        data_yuan(data2_L+1:data_L,:) = sign(imag(data_fc));error2 = sum(data_yuan(data2_L+1:end,:)~=Qi);
        error(1,ll) = (sum(error1)+sum(error2))/(data_L*N_48);%%计算误码率
    end
...
%%
%滤波成形
function [ sig_out ] = lvbo_shape_shen( sig_in )
Fd = 1;Fss = 2;
sig_out = rcosflt(sig_in,Fd,Fss,'fir',Roll,Delay);
sig_out(1:Delay*Fss/Fd,:)=[];sig_out(end-Delay*Fss/Fd+1:end,:)=[];
end
%%
%生成QPSK信号
function [ Ii,Qi ] = create_signal( data_L,N_48 )
data = 2*randsrc(data_L,N_48,[0,1])-1;        %产生48路信号
for m1 = 1:N_48                                  % I、Q分路
...
data_r = Send_dataa;
for k4=1:frame_num
    data_r_temp = rcosflt(Send_dataa(5000+1+frame_len*(k4-1):5000+head+frame_len*(k4-1)),Fd,Fss,'fir/sqrt/Fs',Roll,Delay);
    data_r_temp(1:Delay*Fss/Fd)=[];
    data_r_temp(end-Delay*Fss/Fd+1:end)=[];
    data_r(5000+1+frame_len*(k4-1):5000+head+frame_len*(k4-1)) = (data_r_temp).';
end
end
%%
%捕获
