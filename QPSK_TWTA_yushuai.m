...
Io = lvbo_shape(Ii+1i*Qi);                                      %成形滤波，两倍上采样
...
%加入行波管放大器
shape_data = frameI_s;
model = 1; Br = chose_Br(model);                                %三种参数的行波管放大器，对于model=1,2,3
shape_data = sqrt(1/(4*Br))*shape_data./(1.6);
PA_pre_out=presaleh(shape_data,model);                          %加行波管前的预失真
PA_out = TWTA_salehs(PA_pre_out,1,model);                       %信号通过行波管放大器

for nn = 1:1                                                            %nn:选择天气数目
snr = 1:1:10
for kk = 1:length(snr)
    for ll = 1:100
        SNR=10^((snr(kk)+10*log10(2)+10*log10(1/1.25))/10);             %信噪比
        fd=randint(1,1,[-30000,30000]);Delta_Phase=rand(1)*2*pi;        %随机初相，Rad
        send_data=PA_out.*exp(1i*(2*pi*(fd)*[1:length(shape_data)]*1/Fs+Delta_Phase)) ;%%加频偏
        send_data = send_data.*Ka_channel(length(send_data),nn);        %加雨衰信道
        ...
        thr =3e6;dataa = buhuo(real(data_r),imag(data_r),thr);          %同步捕获
