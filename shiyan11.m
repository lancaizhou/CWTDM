...
for k = 1:length(data_r)
    ...
    if (abs(rI1+1i*rQ1)>50)&&(abs(rI2+1i*rQ2)>50)
...
% 
% mmedian = median([dphasee11,dphasee12,dphasee13]);
% if mmedian==dphasee11
%     dphasee= (dphasee12+dphasee13)/2;
% end
% if mmedian==dphasee12
%     dphasee= (dphasee11+dphasee13)/2;
% end
% if mmedian==dphasee13
%     dphasee= (dphasee11+dphasee13)/2;
% end
% 
% % dphasee = (dphasee_temp+df)/2;
% 
% 
% %%%%%补偿粗估计频偏
...
%数据匹配滤波
data_f = rcosflt(data_f,Fb,Fs,'fir/sqrt/Fs',R,Delay);
data_f(1:Delay*Fss/Fd,:)=[];   
data_f(end-Delay*Fss/Fd+1:end,:)=[];
